...
import random
import time
# from base64 import b64decode            # ✂️ не нужно, PNG не сохраняем
# from io import BytesIO                  # ✂️ не нужно, PNG не сохраняем
from pathlib import Path

# from PIL import Image                   # ✂️ не нужно, PNG не сохраняем
...
# IMG_DIR = OUT_DIR / "phones"            # ✂️ не используем
# IMG_DIR.mkdir(exist_ok=True)            # ✂️ не используем
...
def extract_phone_image_data(item, page: Page, avito_id: str) -> str | None:
    ...
    # ✂️ Отключено: сохранение PNG на диск
    # try:
    #     header, b64_data = src.split(",", 1)
    #     raw = b64decode(b64_data)
    #     image = Image.open(BytesIO(raw)).convert("RGB")
    #     file_name = f"{avito_id or int(time.time())}.png"
    #     image.save(IMG_DIR / file_name)
    #     print(f"💾 [{avito_id}] PNG сохранён: {IMG_DIR / file_name}")
    # except Exception as e:
    #     print(f"⚠️ [{avito_id}] Ошибка при сохранении PNG: {e}")
    #     pass

    # Возвращаем data-uri, чтобы положить в JSON
    return src
    ...
    with sync_playwright() as p:
        ...
        for idx, item in enumerate(items, start=1):
            ...
            try:
                ...
                if data_uri:
                    phones_map[url] = data_uri  # <-- в JSON пойдёт data:image/png;base64,...
                    found_count += 1
        ...
        # print(f"📂 PNG лежат в {IMG_DIR}")  # ✂️ больше не сохраняем PNG
...
#     """
#     После клика ищем img[data-marker='phone-image'],
#     возвращаем data:image/png;base64,... (и сохраняем PNG).
#     """
#
#     # сначала ищем в пределах карточки
#     try:
...
#     except Exception as e:
#         print(f"⚠️ [{avito_id}] Ошибка при сохранении PNG: {e}")
#         # даже если не сохранили файл, сам data-uri вернуть можно
#         pass
#
#     return src  # если хочешь хранить путь, верни file_name вместо src
#
#
# def parse_phone_image_for_item(page: Page, item, idx_on_page: int) -> str | None:
#     """
